...
IOS_BUILD_ARMV7_CMD = 'cmake ../.. -DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE=../../ios.toolchain.cmake -DIOS_PLATFORM=OS -DIOS_ARCH="armv7" -DENABLE_ARC=0 -DENABLE_BITCODE=0 -DENABLE_VISIBILITY=1 && make -j8 && make install'

GEN_IOS_OS_PROJ = 'cmake ../.. -G Xcode -DCMAKE_TOOLCHAIN_FILE=../../ios.toolchain.cmake -DIOS_PLATFORM=OS -DIOS_ARCH="armv7" -DENABLE_ARC=0 -DENABLE_BITCODE=0 -DENABLE_VISIBILITY=1'
OPEN_SSL_ARCHS = ['x86_64', 'arm64', 'armv7']
...
def build_ios(tag=''):
    ...
    ret = os.system(IOS_BUILD_ARMV7_CMD)
    os.chdir(SCRIPT_PATH)
    if ret != 0:
        print('!!!!!!!!!!!build armv7 fail!!!!!!!!!!!!!!!')
        return False
    
    libtool_armv7_dst_lib = INSTALL_PATH + '/armv7'
    if not libtool_libs(libtool_src_lib, libtool_armv7_dst_lib):
        return False
