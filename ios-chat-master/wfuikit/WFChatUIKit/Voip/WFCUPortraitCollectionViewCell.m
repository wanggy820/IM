...
- (void)onVolumeUpdated:(NSNotification *)notification {
    ...
    if (volume > 10 && !self.profile.audioMuted) {
...
-(void)setProfile:(TJAVParticipantProfile *)profile {
    _profile = profile;
    if (profile.state == kTJAVEngineStateConnected || profile.state == kTJAVEngineStateIdle) {
