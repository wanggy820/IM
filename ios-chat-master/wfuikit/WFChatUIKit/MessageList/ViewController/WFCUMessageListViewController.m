...
- (void)didTouchVideoBtn:(BOOL)isAudioOnly {
    if(self.conversation.type == Single_Type) {
        ...
        vc.type = Vertical;
        vc.maxSelectCount = (int)(candidateUser.count > 9 ? 9 : candidateUser.count);
        __weak typeof(self)ws = self;
        vc.selectResult = ^(NSArray<NSString *> * _Nonnull contacts) {
            UIViewController *videoVC = [[WFCUMultiVideoViewController alloc] initWithTargets:contacts conversation:ws.conversation audioOnly:isAudioOnly];
            videoVC.modalPresentationStyle = UIModalPresentationFullScreen;
            [ws presentViewController:videoVC animated:YES completion:nil];
        };
