...
+ (CGSize)sizeForClientArea:(WFCUMessageModel *)msgModel withViewWidth:(CGFloat)width {
    NSString *text = [WFCUCallSummaryCell getCallText:msgModel.message];
...
+ (NSString *)getCallText:(WFCCMessage *)message {
    WFCCCallStartMessageContent *startContent = (WFCCCallStartMessageContent *)message.content;
    NSString *text;
    ...
    } else if (message.messageId == TJIMSDK.shareSDK.message.messageId && TJIMSDK.shareSDK.state == kTJAVEngineStateConnected) {
        text = @"对话中";
    } else {
        switch (startContent.status) {
            case kTJAVCallEndReasonUnknown:
                text = @"未接通";
                break;
            case kTJAVCallEndReasonBusy:
                text = @"线路忙";
                break;
            case kTJAVCallEndReasonSignalError:
                text = @"网络错误";
                break;
            case kTJAVCallEndReasonHangup:
                text = @"已取消";
                break;
            case kTJAVCallEndReasonMediaError:
                text = @"网络错误";
                break;
            case kTJAVCallEndReasonRemoteHangup:
                text = @"对方已取消";
                break;
            case kTJAVCallEndReasonOpenCameraFailure:
                text = @"网络错误";
                break;
            case kTJAVCallEndReasonTimeout:
                text = @"未接听";
                break;
            case kTJAVCallEndReasonAcceptByOtherClient:
                text = @"其它端已接听";
                break;
            case kTJAVCallEndReasonAllLeft:
                text = @"通话已结束";
                break;
            case kTJAVCallEndReasonRemoteBusy:
                text = @"对方线路忙";
                break;
            case kTJAVCallEndReasonRemoteTimeout:
                text = @"对方未接听";
                break;
            case kTJAVCallEndReasonRemoteNetworkError:
                text = @"对方网络错误";
                break;
            case kTJAVCallEndReasonRoomDestroyed:
                text = @"通话已结束";
                break;
            case kTJAVCallEndReasonRoomNotExist:
                text = @"通话已结束";
                break;
            case kTJAVCallEndReasonRoomParticipantsFull:
                text = @"已达到最大参与人数";
                break;
            default:
                break;
...
- (void)setModel:(WFCUMessageModel *)model {
    ...
    self.infoLabel.text = [WFCUCallSummaryCell getCallText:model.message];
