...
- (void)viewDidLoad {
    ...
    if (![[ShareAppService sharedAppService] isLogin]) {
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"没有登录" message:@"请先登录老板云IM" preferredStyle:UIAlertControllerStyleAlert];
        ...
        if ([provider hasItemConformingToTypeIdentifier:@"public.file-url"]) {
            [provider loadItemForTypeIdentifier:@"public.file-url" options:nil completionHandler:^(__kindof id<NSSecureCoding>  _Nullable item, NSError * _Null_unspecified error) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    fileLabel.text = [fileName stringByRemovingPercentEncoding];
                    ws.dataLoaded = YES;
                });
            }];
...
- (CGSize)getTextDrawingSize:(NSString *)text
    ...
    return [text boundingRectWithSize:constrainedSize options:(NSStringDrawingTruncatesLastVisibleLine |  NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading) attributes:@{NSFontAttributeName : font} context:nil].size;
}
    ...
    UIAlertAction *action = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        if (ws.textMessageContent.length) {
            ...
            [[ShareAppService sharedAppService] uploadFiles:ws.imageUrls[0] mediaType:1 fullImage:self.fullImage progress:^(int sentcount, int dataSize) {
                ...
                [[ShareAppService sharedAppService] sendImageMessage:conversation mediaUrl:url thubnail:thumbnail success:^(NSDictionary * _Nonnull dict) {
                    [ws showSuccess];
                } error:^(NSString * _Nonnull message) {
                    [ws showFailure];
                }];
...
- (void)showImageLimit {
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"不支持发送多张图片" message:@"每次只能发送一张。如果您需要一次发送多张，请打开老板云IM选择图片发送。" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)showSuccess {
    ...
    __weak typeof(self)ws = self;
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"已发送" message:@"您可以在老板云IM中查看" preferredStyle:UIAlertControllerStyleAlert];
