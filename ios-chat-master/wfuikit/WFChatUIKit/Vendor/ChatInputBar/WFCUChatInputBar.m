...
- (UIView *)pluginInputView {
    if (!_pluginInputView) {
        BOOL hasVoip = (self.conversation.type == Single_Type) || (self.conversation.type == Group_Type);
...
- (void)onItemClicked:(NSUInteger)itemTag {
    ...
    self.inputBarStatus = ChatInputBarDefaultStatus;
    if (itemTag == 1) {
        ZLPhotoActionSheet *ps = [[ZLPhotoActionSheet alloc] init];
...
//        [ZLPhotoConfiguration defaultPhotoConfiguration].allowEditVideo = YES;
        ZLCustomCamera *cc = [[ZLCustomCamera alloc] init];
        cc.doneBlock = ^(UIImage * _Nullable image, NSURL * _Nullable url) {
...
- (void)handleVideo:(NSURL *)url photos:(NSMutableArray<PHAsset *> *)photos isFullImage:(BOOL)isFullImage {
    ...
    CGImageRef oneRef = [generate1 copyCGImageAtTime:time actualTime:NULL error:&err];
    UIImage *thumbnail = [[UIImage alloc] initWithCGImage:oneRef];
    thumbnail = [WFCCUtilities generateThumbnail:thumbnail withWidth:240 withHeight:240];
